<div id="page-wrapper">
    <div class="container-fluid">
        <div class="row bg-title">
            <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                <h2 class="page-title">KELOLA USER</h2> </div>
            <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                <ol class="breadcrumb">
                    <li><a href="#">Dashboard > Kelola User</a></li>
                </ol>
            </div>
            <!--<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                <ol class="breadcrumb">
                    <li><a href="#">Taaruf Semua Orang</a></li>
                </ol>
            </div>-->
            <!-- /.col-lg-12 -->
        </div>
        <div class="row">
            <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
                <div class="white-box">
                    <h3 class="box-title">Daftar User</h3>
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>Nama</th>
                                    <th>Umur</th>
                                    <th>Tempat Lahir</th>
                                    <th>Tanggal Lahir</th>
                                    <th>Asal</th>
                                    <th>Domisili</th>
                                    <th>Pekerjaan</th>
                                </tr>
                            </thead>
                            <tbody>
                                 <tr>
                                    <td class="txt-oflo">Rohaya Husna</td>
                                    <td class="txt-oflo">21</td>
                                    <td class="txt-oflo">Solo</td>
                                    <td class="txt-oflo">13 Juli 1995</td>
                                    <td class="txt-oflo">Solo</td>
                                    <td class="txt-oflo">Solo</td>
                                    <td class="txt-oflo">Karyawan</td>
                                </tr>
                                <tr>
                                    <td class="txt-oflo">Latifah Nur</td>
                                    <td class="txt-oflo">22</td>
                                    <td class="txt-oflo">Surabaya</td>
                                    <td class="txt-oflo">29 Januari 1995</td>
                                    <td class="txt-oflo">Surabaya</td>
                                    <td class="txt-oflo">Jakarta</td>
                                    <td class="txt-oflo">Kontraktor</td>
                                </tr>
                                <tr>
                                    <td class="txt-oflo">Sulton Ramadan</td>
                                    <td class="txt-oflo">25</td>
                                    <td class="txt-oflo">Surabaya</td>
                                    <td class="txt-oflo">9 Juni 1991</td>
                                    <td class="txt-oflo">Surabaya</td>
                                    <td class="txt-oflo">Surabaya</td>
                                    <td class="txt-oflo">Guru</td>
                                    
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
      
                </div>
            </div>
        </div>
        
    </div>
    <!-- /.container-fluid -->
    <footer class="footer text-center"></footer>
</div>